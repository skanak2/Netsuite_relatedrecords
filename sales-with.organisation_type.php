...
$search->savedSearchId ="1414"; //Saved Search internal ID, where the internal id's of all sales order records are saved
$cnt=0;
...
if (!$searchResponse->searchResult->status->isSuccess) {
    ...
    foreach ($records as $record)  {
    	...
    	$searchedRowItemJoin = $record->customerMainJoin->customFieldList;
    	//var_dump($searchedRowItemJoin);
    	
    	
    	for($h=0;$h<3;$h++)// to tale organisation type
    	{
    	if(isset($searchedRowItemJoin->customField[$h])){
    	if($searchedRowItemJoin->customField[$h]->scriptId=="custentitycustomer_organizationtype"){
    				...
    				for($f=0;$f<20;$f++)
    				{
    					if(isset($my_custom_lists[$mycust]->customValue[$f])){
    						if($my_custom_lists[$mycust]->customValue[$f]->valueId==$category1)
    							$organisationtype=$my_custom_lists[$mycust]->customValue[$f]->value;
       ...
       // var_dump($record->basic);
        	...
        	echo "oooooooooooooooooooo typeeeee";
        	var_dump($organisationtype);
